...
* MotionMark and similar vector benchmarks must use public `DrawingContext` APIs with retained `PathGeometry`. Drive attached samples through `IAnimatedElement`, invalidate before compilation, and keep `OnRender` scheduling-free. Geometry generation may be O(N) when data or viewport changes; the time-normalized split update is O(K) for K due toggles, grouped recording is O(N + G), and segment objects, brushes, pens, HUD resources, and pooled group containers must be reused with no per-segment managed allocation after warmup.
